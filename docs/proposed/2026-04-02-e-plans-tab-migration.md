...
**Principles applied**: Four afterthoughts found and fixed. (1) CLI commands (`MarkdownPlannerExecuteCommand`, `MarkdownPlannerPlanCommand`) were still using the deprecated use cases while the Mac app had been updated to `MarkdownPlannerService` — updated both. (2) `PipelineView` defined its own nested `Phase` struct that duplicated `PlanPhase`, causing conversion boilerplate in `phaseSection`; `MarkdownPlannerModel` still used `PhaseStatus` instead of `PlanPhase` — removed `PipelineView.Phase`, updated `PipelineView` to accept `[PlanPhase]`, and updated `ExecutionProgress.phases`, `State.completed`, `lastExecutionPhases`, and `phaseCompleted` handler to use `[PlanPhase]` consistently. (3) `PlanGenerationNode.swift` was placed at the feature root while all other helpers live in `services/` — moved it. (4) `PipelineView.swift` was isolated in its own `Views/Pipeline/` directory instead of `Views/Components/` alongside `OutputPanel` — moved it.
...
**Skills used**: `ai-dev-tools-review`, `swift-app-architecture:swift-architecture`
**Principles applied**: Two violations against the Features layer reference fixed: (1) `MarkdownPlannerService` in `MarkdownPlannerFeature` did not conform to `UseCase`/`StreamingUseCase` while every other Features type does — added `UseCase` conformance. (2) `PlanPhase` was defined in the Features layer but used as a shared model by Apps layer views (`PipelineView`, `ActivePlanModel`) — shared model types belong in Services; moved `PlanPhase.swift` from `MarkdownPlannerFeature/services/` to `MarkdownPlannerService/` (Services layer), updated `PipelineView` to import `MarkdownPlannerService` instead of `MarkdownPlannerFeature`, and added the Services import to other consumers. All Apps layer files correctly follow the apps-layer reference.
...
**Skills used**: `ai-dev-tools-review`, `swift-app-architecture:swift-architecture`
**Principles applied**: One SDK-layer violation found and fixed: `MarkdownTaskSourceState` was a mutable `final class` with a lock and boolean flag held inside `MarkdownTaskSource` (SDKs layer). The SDK reference requires stateless `Sendable` structs with no mutable internal state, classes, or actors. The class existed to prevent `nextTask(taskIndex:)` from returning the same task twice, but was unnecessary — `nextTask()` re-reads the file on every call via `MarkdownPipelineSource.load()`, so checking `!$0.isCompleted` in the indexed branch is sufficient. Removed `MarkdownTaskSourceState` entirely and added the `isCompleted` check, making `MarkdownTaskSource` a properly stateless struct.
...
**Skills used**: `ai-dev-tools-review`, `swift-app-architecture:swift-architecture`
**Principles applied**: Audited all layers for upward imports. Checked every Services module for Feature/App imports, every SDKs module for Services/Feature/App imports, and every Features module for App imports. Also checked for feature-to-feature lateral imports (prohibited). All layers are clean — no upward dependencies found. The migration correctly maintains the downward-only dependency flow: Apps → Features → Services → SDKs.
...
**Principles applied**: Audited all files changed in phases 1-4 for `@Observable` and `@MainActor` outside the Apps layer. No violations found — every instance of these annotations appears in `Apps/AIDevToolsKitMac/` (models: `MarkdownPlannerModel`, `ActivePlanModel`; views: `MarkdownPlannerDetailView`). The newly introduced `MarkdownPlannerService`, `PlanGenerationNode`, `WatchPlanUseCase`, `MarkdownTaskSource`, and `PlanPhase` are all free of these annotations. Pre-existing `@MainActor` uses in `ArchitecturePlannerFeature` use cases and `ArchitecturePlannerStore` (Services layer) are out of scope for this migration.
...
**Skills used**: `ai-dev-tools-review`, `swift-app-architecture:swift-architecture`
**Principles applied**: Two orchestration violations found and fixed. (1) `MarkdownPlannerModel.appendReviewTemplate()` performed 3-step orchestration (load steps via `ReviewTemplateService` → map to `CodeChangeStep` → append via `MarkdownPipelineSource`) directly in the App layer model — extracted to `AppendReviewTemplateUseCase` in `MarkdownPlannerFeature/usecases/`; model now calls the use case with one line. (2) `MarkdownPlannerPlanCommand` with `--execute` invoked `MarkdownPlannerExecuteCommand.parse([...])` and then `.run()` — App layer calling App layer, bypassing the service entirely — replaced with a direct call to `MarkdownPlannerService.execute()` with the same `TimerDisplay`-backed progress handler; `handleProgress` visibility changed from `private static` to `static` to allow reuse within the CLI target.
