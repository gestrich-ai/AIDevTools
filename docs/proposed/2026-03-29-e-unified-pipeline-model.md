...
## - [ ] Phase 1: Define the Core Pipeline Protocols (SDKs layer)
...
A configuration enum or init parameter selects which format to parse. Steps parsed from markdown become `CodeChangeStep` instances. The `CreatePRStep` is not represented in markdown today — ClaudeChain appends it implicitly at the end; `MarkdownPipelineSource` should append it automatically when the ClaudeChain format is detected (or based on a config flag).
