...
struct FlatTaskLevelCachingTests {
    ...
    func additiveTaskFileWrites() throws {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            ...
            .appendingPathComponent(PRRadarPhasePaths.prepareTasksSubdir)
        ...
        let existingFilePath = tmpDir.appendingPathComponent("\(PRRadarPhasePaths.dataFilePrefix)\(existingTask.taskId).json")
        ...
        let newFilePath = tmpDir.appendingPathComponent("\(PRRadarPhasePaths.dataFilePrefix)\(newTask.taskId).json")
        ...
        #expect(files.contains("\(PRRadarPhasePaths.dataFilePrefix)existing-rule_file-foo-1-5_swift-rules.json"))
        #expect(files.contains("\(PRRadarPhasePaths.dataFilePrefix)new-rule_file-bar-1-5_security-rules.json"))
    ...
    func focusAreaCacheLoading() throws {
        ...
        let filePath = tmpDir.appendingPathComponent("\(PRRadarPhasePaths.dataFilePrefix)file.json")
        ...
        let existingFiles = try FileManager.default.contentsOfDirectory(atPath: tmpDir.path)
            .filter { $0.hasPrefix(PRRadarPhasePaths.dataFilePrefix) && $0.hasSuffix(".json") }
    ...
    func rulesFilePerDirectoryNaming() {
        // Arrange & Act
        let filenameA = PRRadarPhasePaths.rulesFilename(forRulesDir: "/rules/swift-rules")
        let filenameB = PRRadarPhasePaths.rulesFilename(forRulesDir: "/rules/security-rules")
    ...
    func isRulesFileMatchesPerDirFiles() {
        #expect(PRRadarPhasePaths.isRulesFile("rules-swift-rules.json"))
        #expect(PRRadarPhasePaths.isRulesFile("rules-security-rules.json"))
        #expect(!PRRadarPhasePaths.isRulesFile("data-task1.json"))
        #expect(!PRRadarPhasePaths.isRulesFile("all-rules.json"))
    }
