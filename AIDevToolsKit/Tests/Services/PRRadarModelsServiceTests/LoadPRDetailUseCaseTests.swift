...
struct LoadPRDetailUseCaseTests {
    ...
    private func makeConfig(outputDir: String) -> RepositoryConfiguration {
        RepositoryConfiguration(name: "test", repoPath: "/tmp/fake-repo", outputDir: outputDir, rulePaths: [RulePath(name: "default", path: "/tmp/rules", isDefault: true)], agentScriptPath: "/tmp/agent.py", githubAccount: "test", defaultBaseBranch: "main")
    }
    ...
    private func setupFullPR(outputDir: String, prNumber: Int, commitHash: String) throws {
        ...
        try writeJSON(diff, to: "\(diffDir)/\(PRRadarPhasePaths.diffParsedJSONFilename)")
        try writeJSON(diff, to: "\(diffDir)/\(PRRadarPhasePaths.effectiveDiffParsedJSONFilename)")
        ...
        try writeJSON(prDiff, to: "\(diffDir)/\(PRRadarPhasePaths.prDiffFilename)")
        ...
        try writeJSON([rule], to: "\(prepareRulesDir)/\(PRRadarPhasePaths.rulesFilename(forSlug: "test"))")
        ...
        try writeJSON(summary, to: "\(evaluateDir)/\(PRRadarPhasePaths.summaryJSONFilename)")
        ...
        try writeJSON(report, to: "\(reportDir)/\(PRRadarPhasePaths.summaryJSONFilename)")
        try "# Report\n".write(toFile: "\(reportDir)/\(PRRadarPhasePaths.summaryMarkdownFilename)", atomically: true, encoding: .utf8)
        ...
        try writeJSON(["img1.png": "https://example.com/img1.png"], to: "\(metadataDir)/\(PRRadarPhasePaths.imageURLMapFilename)")
        ...
        try writeJSON(pr, to: "\(metadataDir)/\(PRRadarPhasePaths.ghPRFilename)")
    }
    ...
    func loadImageMap() throws {
        ...
        try writeJSON(map, to: "\(metadataDir)/\(PRRadarPhasePaths.imageURLMapFilename)")
