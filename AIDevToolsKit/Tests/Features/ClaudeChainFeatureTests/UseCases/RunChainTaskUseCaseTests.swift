...
struct RunSpecChainTaskUseCaseTests {
    ...
    func summaryAndCommentAfterPRCreation() {
        // The implementation in RunSpecChainTaskUseCase.run() executes phases sequentially:
        //   finalize → prCreated → generatingSummary → summaryCompleted → postingPRComment → prCommentPosted → completed
        //
        // This ordering is verified by code inspection of RunSpecChainTaskUseCase.swift lines 212-287:
...
private func progressLabel(_ progress: RunSpecChainTaskUseCase.Progress) -> String {
    switch progress {
...
@Suite("RunSpecChainTaskUseCase.appendReviewNote")
struct AppendReviewNoteTests {

    private let useCase = RunSpecChainTaskUseCase(client: StubAIClient())
