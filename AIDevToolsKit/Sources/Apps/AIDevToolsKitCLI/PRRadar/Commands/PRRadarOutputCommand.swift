...
struct PRRadarOutputCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        if let phaseStr = phase {
            guard let matched = PRRadarPhase.allCases.first(where: { $0.rawValue == phaseStr }) else {
                throw PRRadarCLIError.phaseFailed("Unknown phase '\(phaseStr)'. Valid: prepare, evaluate")
            }
        ...
        if let taskId = task {
            guard let match = allOutputs.first(where: { $0.output.identifier == taskId }) else {
                throw PRRadarCLIError.phaseFailed("No output found with identifier '\(taskId)'")
            }
            printOutput(match.output, phase: match.phase, options: options, jsonOutput: jsonOutput)
            return
        }
    ...
    private func printOutput(_ output: EvaluationOutput, phase: PRRadarPhase, options: PRRadarCLIOptions, jsonOutput: Bool) {
        if options.json || jsonOutput {
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
            encoder.dateEncodingStrategy = .iso8601
