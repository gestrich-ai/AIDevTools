...
struct PRRadarFilterOptions: ParsableArguments {
    ...
    func buildFilter(config: RepositoryConfiguration) throws -> PRFilter {
        ...
        if let updatedSince {
            guard let date = parsePRRadarDateString(updatedSince) else {
            ...
            guard let date = parsePRRadarDateString(since) else {
        ...
        let stateFilter: PRState? = try parsePRRadarStateFilter(state)
