...
public struct ParseEventCommand: AsyncParsableCommand {
    ...
    ///
    /// - Parameters:
    ///   - service: ClaudeChainService for local project discovery
        ...
        do {
            ...
            if context.eventName == "workflow_dispatch" {
                ...
                if let repo = repo, let prNumber = context.prNumber {
                    let detectedProjects = try await detectProjectsFromPRFiles(service: service, prNumber: prNumber, repo: repo)
                ...
                if let repo = repo {
                    let detectedProjects = try await detectProjectsFromChangedFiles(service: service, context: context, repo: repo)
                    resolvedProject = selectProjectAndOutputAll(gh: gh, projects: detectedProjects)
                }
    ...
    private func detectProjectsFromChangedFiles(service: ClaudeChainService, context: GitHubEventContext, repo: String) async throws -> [Project] {
    ...
    /// - Returns: List of Project objects for projects with changed spec.md files.
    ///            Empty list if no spec files were changed or detection failed.
    private func detectProjectsFromPRFiles(service: ClaudeChainService, prNumber: Int, repo: String) async throws -> [Project] {
