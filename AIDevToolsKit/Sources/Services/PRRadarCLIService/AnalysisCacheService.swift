...
public enum AnalysisCacheService {
    ...
    private static func lookupCachedResult(
        ...
        let evalPath = "\(evalsDir)/\(PRRadarPhasePaths.dataFilePrefix)\(task.taskId).json"
    ...
    private static func lookupCrossCommitResult(
        ...
        for priorDir in priorEvalsDirs {
            ...
            let evalFilename = "\(PRRadarPhasePaths.dataFilePrefix)\(task.taskId).json"
            ...
            // Copy evaluation output files so Output view can display them
            let outputBase = "output-\(task.taskId)"
            copyFile(from: "\(priorDir)/\(outputBase).json", to: "\(targetEvalsDir)/\(outputBase).json")
            copyFile(from: "\(priorDir)/\(outputBase).md", to: "\(targetEvalsDir)/\(outputBase).md")
    ...
    static func findPriorEvalsDirs(
        ...
        let analysisDir = "\(prOutputDir)/\(PRRadarPhasePaths.analysisDirectoryName)"
