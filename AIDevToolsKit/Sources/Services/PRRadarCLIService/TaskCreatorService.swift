...
public struct TaskCreatorService: Sendable {
    ...
    public func createAndWriteTasks(
        ...
        let tasksDir = "\(outputDir)/\(PRRadarPhasePaths.prepareTasksSubdir)"
        try PRRadarPhasePaths.ensureDirectoryExists(at: tasksDir)
        ...
        for task in tasks {
            ...
            let filePath = "\(tasksDir)/\(PRRadarPhasePaths.dataFilePrefix)\(task.taskId).json"
