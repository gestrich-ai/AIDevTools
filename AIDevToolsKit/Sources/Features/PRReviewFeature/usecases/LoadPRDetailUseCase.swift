...
public struct LoadPRDetailUseCase: UseCase {
    ...
    public func execute(prNumber: Int, commitHash: String? = nil) async -> PRDetail {
        ...
        if let map: [String: String] = try? PhaseOutputParser.parsePhaseOutput(
            ...
            prNumber: prNumber,
            phase: .metadata,
            filename: PRRadarPhasePaths.imageURLMapFilename
        ) {
            imageURLMap = map
    ...
    private func loadSavedOutputs(prNumber: Int, commitHash: String?) -> [PRRadarPhase: [EvaluationOutput]] {
        ...
        let prepareFiles = PhaseOutputParser.listPhaseFiles(
            ...
            subdirectory: PRRadarPhasePaths.prepareFocusAreasSubdir, commitHash: commitHash
        )
        let prepareOutputs = loadOutputs(
            from: prepareFiles, prNumber: prNumber, phase: .prepare,
            subdirectory: PRRadarPhasePaths.prepareFocusAreasSubdir, commitHash: commitHash, decoder: decoder
        )
