...
public struct PrepareUseCase: StreamingUseCase {
        ...
        AsyncThrowingStream<PhaseProgress<PrepareOutput>, Error>(PhaseProgress<PrepareOutput>.self) { continuation in
            ...
            Task {
                do {
                    ...
                    let focusDir = PRRadarPhasePaths.phaseSubdirectory(
                    ...
                    if !existingFocusFiles.isEmpty {
                        ...
                        for (focusType, result) in focusResults {
                            ...
                            try data.write(to: URL(fileURLWithPath: "\(focusDir)/\(PRRadarPhasePaths.dataFilePrefix)\(focusType.rawValue).json"))
                        }
                    ...
                    for rulesDir in validRulesDirs {
                        ...
                        let rulesFilePath = try PRRadarPhasePaths.rulesFilePath(
                    ...
                    let tasksDir = PRRadarPhasePaths.phaseSubdirectory(
                        outputDir: config.resolvedOutputDir, prNumber: prNumber,
                        phase: .prepare, subdirectory: PRRadarPhasePaths.prepareTasksSubdir,
                        commitHash: resolvedCommit
                    )
                    let totalTasksOnDisk = Self.countDataFiles(inDirectory: tasksDir)
                    let rulesSubdir = PRRadarPhasePaths.phaseSubdirectory(
                        outputDir: config.resolvedOutputDir, prNumber: prNumber,
                        phase: .prepare, subdirectory: PRRadarPhasePaths.prepareRulesSubdir,
                        commitHash: resolvedCommit
                    )
    ...
    private static func countRulesFiles(inDirectory dir: String) -> Int {
        ((try? FileManager.default.contentsOfDirectory(atPath: dir)) ?? [])
            .filter { PRRadarPhasePaths.isRulesFile($0) }
            .count
    }
    ...
    public static func parseOutput(config: RepositoryConfiguration, prNumber: Int, commitHash: String? = nil) async throws -> PrepareOutput {
        ...
        for file in focusFiles {
            let typeOutput: FocusAreaTypeOutput = try PhaseOutputParser.parsePhaseOutput(
                config: config, prNumber: prNumber, phase: .prepare, subdirectory: PRRadarPhasePaths.prepareFocusAreasSubdir, filename: file, commitHash: resolvedCommit
            )
            allFocusAreas.append(contentsOf: typeOutput.focusAreas)
        }
        ...
        for file in rulesFiles {
            let fileRules: [ReviewRule] = try PhaseOutputParser.parsePhaseOutput(
                config: config, prNumber: prNumber, phase: .prepare, subdirectory: PRRadarPhasePaths.prepareRulesSubdir, filename: file, commitHash: resolvedCommit
            )
