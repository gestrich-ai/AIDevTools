...
public struct MarkdownPipelineSource: PipelineSource {
    ...
    public init(fileURL: URL, format: MarkdownPipelineFormat, appendCreatePRStep: Bool) {
    ...
    public func load() async throws -> Pipeline {
        ...
        for line in content.components(separatedBy: "\n") {
            if let step = try parseStep(from: line, index: index) {
                steps.append(step)
                index += 1
            }
        }
    ...
    public func markStepCompleted(_ step: any PipelineStep) async throws {
        ...
        let content = try String(contentsOf: fileURL, encoding: .utf8)
        let updated = try markCompleted(in: content, stepDescription: step.description)
    ...
    private func parseStep(from line: String, index: Int) throws -> CodeChangeStep? {
        switch format {
        case .phase:
            if line.hasPrefix("## - [x] ") {
    ...
    private func markCompleted(in content: String, stepDescription: String) throws -> String {
        switch format {
        case .phase:
